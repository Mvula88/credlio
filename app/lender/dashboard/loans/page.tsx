...
export default async function LenderLoansPage() {
  ...
  const [
    ...
    supabase.rpc("get_lender_loan_stats", { lender_id: profile.id })
  ])
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          <p className="text-2xl font-bold mt-2">${loanStats?.total_lent || 0}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-sm font-medium text-gray-500">Total Received</h3>
          <p className="text-2xl font-bold mt-2">${loanStats?.total_received || 0}</p>
        </div>
        <div className="bg-white p-6 rounded-lg shadow">
          <h3 className="text-sm font-medium text-gray-500">Avg Interest</h3>
          <p className="text-2xl font-bold mt-2">{loanStats?.avg_interest_rate || 0}%</p>
        </div>
      </div>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        Borrower: {loan.loan_request?.borrower?.full_name}
                      </p>
                      <p className="text-sm text-gray-600">
                        Amount: ${loan.offered_amount} at {loan.interest_rate}% interest
                      </p>
                      <p className="text-sm text-gray-600">
                        Credit Score: {loan.loan_request?.borrower?.credit_score || 'N/A'}
                      </p>
                    </div>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="divide-y divide-gray-200">
                  {completedLoans.map((loan: any) => (
                    <tr key={loan.id}>
                      <td className="px-4 py-2 text-sm">{loan.loan_request?.borrower?.full_name}</td>
                      <td className="px-4 py-2 text-sm">{loan.loan_request?.purpose}</td>
                      <td className="px-4 py-2 text-sm">${loan.offered_amount}</td>
                      <td className="px-4 py-2 text-sm">{loan.interest_rate}%</td>
