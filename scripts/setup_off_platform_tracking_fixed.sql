-- Setup for Off-Platform Defaulters and Ghost Borrower Tracking (FIXED VERSION)
...
-- Risky borrowers from borrower_profiles (check if column exists first)
SELECT 
  ...
  COALESCE(bp.created_at, NOW()) as flagged_at,  -- Use created_at as fallback
  1 as report_count
...
WHERE EXISTS (
  ...
  SELECT id FROM borrower_profiles WHERE 
  CASE 
    WHEN EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'borrower_profiles' AND column_name = 'is_risky')
    THEN (SELECT is_risky FROM borrower_profiles WHERE id = bp.id)
    ELSE false
  END = true
)
