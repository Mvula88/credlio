-- Admin Risk Statistics Functions (FIXED VERSION)
...
BEGIN
  ...
  loan_stats AS (
    SELECT 
      ...
      COALESCE(SUM(CASE WHEN al.status = 'overdue' THEN al.total_amount_due - COALESCE(al.total_amount_paid, 0) END), 0) as overdue_amount
...
DROP MATERIALIZED VIEW IF EXISTS admin_risk_summary;
CREATE MATERIALIZED VIEW admin_risk_summary AS
WITH risk_counts AS (
  SELECT 
    c.code as country_code,
    ...
    -- Revenue (from completed loans)
    COALESCE(SUM(CASE WHEN al.status = 'completed' THEN al.total_amount_paid END), 0) as total_revenue
  FROM countries c
  LEFT JOIN profiles p ON p.country_id = c.id
  LEFT JOIN borrower_profiles bp ON bp.user_id = p.id
  LEFT JOIN blacklists b ON b.borrower_id = p.id AND b.status = 'active'
